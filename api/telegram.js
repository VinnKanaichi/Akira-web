...
async function sendAlert(ip, userAgent, sessionStatus, turnstileStatus, req) {
  try {
    // CEK TOKEN DENGAN KETAT
    if (!ALERT_BOT_TOKEN || ALERT_BOT_TOKEN.length < 10) {
      console.log('⚠️ ALERT_BOT_TOKEN tidak valid');
      return;
    }
    
    if (!ALERT_CHAT_ID) {
      console.log('⚠️ ALERT_CHAT_ID tidak valid');
      return;
    }
...
🛡️ Turnstile: ${turnstileStatus ? '✅ Lolos' : '❌ Gagal'}
    ...
    const response = await fetch(`https://api.telegram.org/bot${ALERT_BOT_TOKEN}/sendMessage`, {
      method: 'POST',
    ...
    if (!response.ok) {
      console.log('⚠️ Alert gagal dikirim:', response.status);
    }
...
export default async function handler(req, res) {
  try {
    ...
    let total = 0;
    try {
      total = await checkRateLimit(ip);
    } catch (err) {
      console.error('Rate limit error:', err);
      total = 0; // default aman
    }
